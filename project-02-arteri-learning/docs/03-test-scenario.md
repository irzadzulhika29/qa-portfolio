...
|---|---|---|---|---|---|
| TS-AUTH-001 | Authentication | Verify guest can login with valid username or email and password | Critical | Functional | Passed |
| TS-AUTH-002 | Authentication | Verify login is rejected when password is invalid | High | Negative | Passed |
...
| TS-RBAC-002 | Role Access | Verify student cannot access admin routes | Critical | Security Basic | Passed |
| TS-RBAC-003 | Role Access | Verify teacher or admin cannot access student-only routes | High | Security Basic | Not Run |
| TS-STUD-001 | Student Dashboard | Verify student dashboard loads greeting, upcoming exam, and active attempt summary | High | Functional | Passed |
| TS-COURSE-001 | Student Courses | Verify student can view enrolled course list | High | Functional | Passed |
| TS-COURSE-002 | Student Courses | Verify course search filters results correctly | Medium | Functional | Not Run |
| TS-COURSE-003 | Student Courses | Verify student can join course with valid enrollment code | High | Functional | Passed |
| TS-CONTENT-001 | Learning Content | Verify student can open course topic and view material content | High | Functional | Passed |
...
| TS-CBT-001 | CBT Exam Access | Verify exam code access returns exam eligibility status correctly | Critical | Functional | Passed |
...
| TS-CBT-003 | CBT Exam Attempt | Verify answers are saved during exam attempt | Critical | Functional | Passed |
...
| TS-CBT-005 | CBT Exam Attempt | Verify student can submit completed exam successfully | Critical | Functional | Passed |
| TS-ADMIN-001 | Admin Dashboard | Verify teacher/admin can access dashboard exam statistics | High | Functional | Passed |
| TS-ADMIN-002 | Course Management | Verify teacher/admin can create new course with valid data | Critical | Functional | Not Run |
| TS-ADMIN-003 | Exam Management | Verify teacher/admin can create exam and add questions | Critical | Functional | Passed |
...
| TS-API-001 | API Auth | Verify protected endpoint rejects request without token | Critical | API Security | Passed |
| TS-API-002 | API Role Access | Verify student token cannot use teacher/admin mutation endpoint | Critical | API Security | Passed |
